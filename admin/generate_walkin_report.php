...
switch ( $type ) {
    case 'yesterday' :
        if( $role == '3' ) {
            ...
            $select_stmt = $mysqli->prepare( "SELECT * FROM orders WHERE type = 'Walk-in' AND DATE(date_added) = CURDATE() - INTERVAL 1 DAY ORDER BY date_added DESC" );
        }else if( $role == '2' ){
            $report_title = '' . $managed_by . ' Yesterday\'s Online Order Sales Report - ' . date( 'M j, Y', strtotime("-1 days") );
            $select_stmt = $mysqli->prepare( "SELECT * FROM orders WHERE type = 'Walk-in' AND DATE(date_added) = CURDATE() - INTERVAL 1 DAY AND managed_by = '$managed_by' ORDER BY date_added DESC" );
        }
        break;
    case 'week' :
        ...
        if( $role == '3' ) {
            ...
            $select_stmt = $mysqli->prepare( "SELECT * FROM orders WHERE type = 'Walk-in' AND YEARWEEK(date_added) = YEARWEEK(CURDATE()) ORDER BY date_added DESC" );
        }else if( $role == '2' ){
            $report_title = '' . $managed_by . ' Week\'s Online Order Sales Report - ' . $dstart . ' - ' . date( 'j, Y' );
            $select_stmt = $mysqli->prepare( "SELECT * FROM orders WHERE type = 'Walk-in' AND YEARWEEK(date_added) = YEARWEEK(CURDATE()) AND managed_by = '$managed_by' ORDER BY date_added DESC" );
        }
        break;
    case 'month' :        
        if( $role == '3' ) {
            $report_title = 'Month\'s Online Order Sales Report - ' . date( 'M, Y' );
            $select_stmt = $mysqli->prepare( "SELECT * FROM orders WHERE type = 'Walk-in' AND MONTH(date_added) = MONTH(CURDATE()) ORDER BY date_added DESC" );
        }else if( $role == '2' ){
    ...
    default :
        if( $role == '3' ) {
            ...
            $select_stmt = $mysqli->prepare( "SELECT * FROM orders WHERE type = 'Walk-in' AND DATE(date_added) = CURDATE() ORDER BY date_added DESC" );
        }else if( $role == '2' ){
            $report_title = '' . $managed_by . ' Today\'s Online Order Sales Report - ' . date( 'M j, Y' );
            $select_stmt = $mysqli->prepare( "SELECT * FROM orders WHERE type = 'Walk-in' AND DATE(date_added) = CURDATE() AND managed_by = '$managed_by' ORDER BY date_added DESC" );
        }
}
$select_stmt->execute();
