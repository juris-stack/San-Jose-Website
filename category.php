...
$site_title = $category_name . ' &mdash; ' . get_siteinfo( 'site-name' );
        ...
        <div class="row">
            <div class="col-md-9 content">
                <div class="row">
                    <?php
                    $paginationCtrls = '';
                    $count_query = $mysqli->query( "SELECT COUNT(ID) FROM products WHERE category=$category_id AND status='published' AND stocks > 0" );
                    ...
                    $get_products_stmt = $mysqli->prepare( "SELECT * FROM products WHERE category=$category_id AND status='published' AND stocks > 0 ORDER BY name ASC $limit" );
